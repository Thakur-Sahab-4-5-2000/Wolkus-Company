...
export default React.memo(function Card({ index, movieData, isLiked = false }) {
  ...
  const addToList = async () => {
    try {
      await axios.post("https://shubham1804592.herokuapp.com/api/user/add", {
        email,
        data: movieData,
      });
    } catch (error) {
  ...
  return (
    ...
    >
      <Link to="/details" params={{ data: movieData }}>
        <img
      ...
      {isHovered && (
        <div className="hover">
          <div className="image-video-container">
            <Link to={`/details/${movieData.id}`} state={{ data: movieData }}>
              <img
                src={`https://image.tmdb.org/t/p/w500${
                  movieData.image || movieData.backdrop_path
                }`}
